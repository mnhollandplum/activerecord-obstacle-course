...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    users = User.joins(:items).where('items.id = 7').distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = User.find(2).orders.joins(:order_items).where(order_items: {item_id: 4})
    # -----------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select("users.name, count(orders.id) AS total_order_count").joins(:orders).group(:id, :user_id).order(:name)
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.all.includes(:order_items).includes(:items)
    # ------------------------------------------------------
